#' @title Subset cryptic alternative 3' splice site (A3SS)
#'
#' @description Subset A3SS splicing events whose distance to the canonical are within a user-defined distance.
#'
...
#'
#' @return An object of class S3 with updated slot \code{$SpliceFeature$A3SS} and new slot \code{$DistanceToCanonical$A3SS}.
#'
...
SubsetCrypticSS.A3SS <- function(MarvelObject, DistanceToCanonical=100) {

    # Define arguments
    df.feature <- MarvelObject$SpliceFeature$A3SS
    ...
    # Report progress
    message(paste(nrow(df.feature.small), " A3SS identified", sep=""))
    message(paste(nrow(df.feature.small.), " cryptic A3SS retained", sep=""))
   
    # Save to new slots
    MarvelObject$SpliceFeature$A3SS <- df.feature.small.
    MarvelObject$DistanceToCanonical$A3SS <- df.feature.small
