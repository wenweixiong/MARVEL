...
#'
#' @description
#' \code{FindPTC.RI.NegStrand} finds PTC(s) introduced by alternative exons into protein-coding transcripts.
#'
#' @details
...
#' df <- df[which(df$event_type=="RI"), ]
#' df <- df[grep(":-@", df$tran_id, fixed=TRUE), ]
#' tran_id <- df$tran_id[1]
#' gene_id <- df$gene_id[1]
#'
#' # Run example
#' results <- FindPTC.RI.NegStrand(MarvelObject=marvel,
#'                                 tran_id=tran_id,
#'                                 gene_id=gene_id
#'                                 )
#'
