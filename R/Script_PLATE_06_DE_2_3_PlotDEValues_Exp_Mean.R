#' @title Plot Differential Gene Expression Analysis Results
#'
#' @description
...
#' @param de.p.val.adj Numeric value. Adjusted p-value below which the genes are considered as statistically significant and will consequently be color-annotated on the plot.
...
#' @export
#'
#' @return An object of class S3 containing all the original slots as inputted by the user in addition to one new slot named \code{MarvelObject$DE$Exp$Plot}.
#'
...
PlotDEValues.Exp <- function(MarvelObject, de.p.val.adj, log2fc, anno=FALSE, anno.x.pos=NULL, anno.y.pos=NULL, anno.x.neg=NULL, anno.y.neg=NULL, label.size=2.5) {
    ...
    df$sig <- NA
    df$sig[which(df$p.val.adj < de.p.val.adj & df$log2fc > log2fc)] <- "up"
    df$sig[which(df$p.val.adj < de.p.val.adj & df$log2fc < (log2fc*-1))] <- "down"
    ...
    if(anno==TRUE) {
        ...
        df$label <- NA
        df$label[which(-log10(df$p.val.adj) > anno.y.pos & df$log2fc > anno.x.pos)] <- df$gene_short_name[which(-log10(df$p.val.adj) > anno.y.pos & df$log2fc > anno.x.pos)]
        df$label[which(-log10(df$p.val.adj) > anno.y.neg & df$log2fc < anno.x.neg)] <- df$gene_short_name[which(-log10(df$p.val.adj) > anno.y.neg & df$log2fc < anno.x.neg)]
