...
PlotValues <- function(MarvelObject, cell.group.list, feature, maintitle="gene_short_name", xlabels.size=8, level, min.cells=NULL, sigma.sq=0.001, bimodal.adjust=NULL, seed=NULL, modality.column="modality.bimodal.adj", scale.y.log=FALSE, max.cells.jitter=10000, max.cells.jitter.seed=1, cell.group.colors=NULL, point.alpha=0.2, point.size=0.001, plot.type="violin", fix.y.axis.interval=TRUE) {
    ...
    if(level=="gene") {
        ...
        PlotValues.PSI(MarvelObject=MarvelObject,
                       ...
                       point.size=point.size,
                       plot.type=plot.type,
                       fix.y.axis.interval=fix.y.axis.interval
                      )
